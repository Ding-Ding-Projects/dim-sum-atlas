...
export async function loadPreparedCatalog(catalogUrl = PUBLIC_CATALOG_URL) {
  const response = await fetch(catalogUrl, { headers: { 'User-Agent': 'Dim-Sum-Atlas/0.1' } });
  if (!response.ok) throw new Error(`Public dim-sum catalog returned HTTP ${response.status}.`);
